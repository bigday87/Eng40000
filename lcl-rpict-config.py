...
key = 'C-|\x19'# arbitrary identification key
#key = '\x43\x2D\x7C\x19'
MAX_NODES = 0
MAX_CHANNELS = 0
...
# If writing then the config file is sent over serial
	...
	parser = optparse.OptionParser(usage="%prog [-w] filename [--version]", version="%prog 1.8.0")
	...
	if os.path.isfile(options.filename):
		...
		if options.filename != "":
			print "%s not a file we can read" % options.filename
			sys.exit()
	...
	if write_config:
		...
		tts += struct.pack('c', chr(int(config.get('main', 'nodeid'))))#nodeid
		...
		if L[4]=='\xa0':
			...
			tts += struct.pack('f', float(config.get('main', 'vest')))#vest
		elif L[4] in ['\xb0','\xb1','\xb2','\xb3','\xc0','\xc1']:
			kcal = config.get('main', 'kcal').split()
			for k in  kcal:
				tts += struct.pack('f', float(k))
			if L[4] in ['\xb3','\xc1']: tts += struct.pack('c', chr(int(config.get('main', 'phasecal'))))#phasecal
			tts += struct.pack('f', float(config.get('main', 'vest')))#vest
